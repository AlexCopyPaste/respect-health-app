// ... (קוד משתנים גלובליים ו-EventListeners נשאר זהה) ...
// ====================
// משתנים גלובליים
// ====================
let charts = {};
...
// ... (פונקציות שמירה saveBP, saveSugar וכו' נשארו זהות לחלוטין) ...
...
// ... (פונקציות עריכה וולידציה נשארו זהות) ...
...
function loadData(type) {
    ...
    list.forEach((item, index) => {
        ...
        // שינוי 4: החלפת סדר כפתורים (עריכה בימין של הפח, פח בקצה שמאל)
        let buttons = `
        ...
        // שינוי 4: תאריך בהתחלה (צד ימין)
        content += `<div class="history-date-full">${dStr}</div>`;

        // נתונים
        if(type === 'bp') {
            // שינוי 3: דופק בסוף (משמאל ללחץ דם)
...
window.toggleChart = function(type) { const wrapper = document.getElementById(type + 'ChartSection'); if(wrapper.classList.contains('hidden')) wrapper.classList.remove('hidden'); else wrapper.classList.add('hidden'); }
window.updateChartFilter = function(type, days) { const key = 'respect_' + type; let list = JSON.parse(localStorage.getItem(key)) || []; const cutoff = new Date(); cutoff.setDate(cutoff.getDate() - days); const filteredList = list.filter(i => new Date(i.date) >= cutoff); updateChart(type, filteredList); }
function updateChart(type, rawData) { const ctx = document.getElementById(type + 'Chart'); if(!ctx) return; if(chartInstances[type]) chartInstances[type].destroy(); const data = [...rawData].reverse(); const labels = data.map(i => { const d = new Date(i.date); return `${d.getDate()}/${d.getMonth()+1}`; }); let datasets = []; if(type==='bp') datasets = [{label:'גבוה', data:data.map(i=>i.sys), borderColor:'red'}, {label:'נמוך', data:data.map(i=>i.dia), borderColor:'orange'}]; else if(type==='weight') datasets = [{label:'משקל', data:data.map(i=>i.val), borderColor:'purple', fill:true, tension:0.4}]; else if(type==='sugar') datasets = [{label:'סוכר', data:data.map(i=>i.val), borderColor:'blue', tension:0.4}]; chartInstances[type] = new Chart(ctx, { type:'line', data:{labels, datasets}, options:{responsive:true, maintainAspectRatio:false, plugins: { legend: { display: true, position: 'top' } } } }); }
function updateDates() { const now = new Date(); now.setMinutes(now.getMinutes() - now.getTimezoneOffset()); const s = now.toISOString().slice(0,16); document.querySelectorAll('input[type="datetime-local"]').forEach(i => i.value = s); const cyc = document.getElementById('cycleStart'); if(cyc) cyc.value = now.toISOString().slice(0,10); }
function loadSettings() { const s = JSON.parse(localStorage.getItem('respect_settings')) || {}; settings = {...settings, ...s}; document.getElementById('largeFontToggle').checked = settings.largeFont; if(settings.largeFont) document.body.classList.add('large-font'); document.getElementById('historyLimitToggle').checked = settings.historyLimit; document.getElementById('exportToggle').checked = settings.showExport; if(localStorage.getItem('theme') === 'dark') { document.body.setAttribute('data-theme', 'dark'); document.getElementById('themeSwitch').checked = true; } }
function toggleSetting(key, val) { if(key === 'theme') { document.body.setAttribute('data-theme', val ? 'dark' : 'light'); localStorage.setItem('theme', val ? 'dark' : 'light'); return; } settings[key] = val; localStorage.setItem('respect_settings', JSON.stringify(settings)); if(key === 'largeFont') document.body.classList.toggle('large-font', val); if(key === 'showExport') updateExportBtn(); if(key === 'historyLimit' && currentSection) loadData(currentSection); }
function updateExportBtn() { const btn = document.getElementById('exportCsvBtn'); if(settings.showExport && currentSection) btn.classList.remove('hidden'); else btn.classList.add('hidden'); }
window.reqDelete = function(type, idx) { document.getElementById('customConfirm').classList.remove('hidden'); pendingDelete = () => { const key = 'respect_' + type; let list = JSON.parse(localStorage.getItem(key)) || []; list.splice(idx, 1); localStorage.setItem(key, JSON.stringify(list)); loadData(type); }; }
window.closeConfirm = function(yes) { document.getElementById('customConfirm').classList.add('hidden'); if(yes && pendingDelete) pendingDelete(); pendingDelete = null; }
window.resetAllData = function() { if(confirm('למחוק את כל הנתונים באפליקציה?')) { localStorage.clear(); location.reload(); } }
window.exportData = function() { if(!currentSection) return; const key = 'respect_' + currentSection; const list = JSON.parse(localStorage.getItem(key)) || []; let csv = '\uFEFFתאריך,נתונים\n'; list.forEach(i => csv += `${i.date||i.start},${JSON.stringify(i).replace(/,/g,' ')}\n`); const a = document.createElement('a'); a.href = 'data:text/csv;charset=utf-8,' + encodeURI(csv); a.download = key + '.csv'; a.click(); }